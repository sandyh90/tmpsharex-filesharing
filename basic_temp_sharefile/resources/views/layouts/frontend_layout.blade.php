...
<body class="d-flex flex-column min-vh-100">
    <div class="wrapper flex-grow-1">
        <nav class="navbar navbar-expand-lg navbar-dark text-white bg-dark py-3">
            <div class="container">
                <a class="navbar-brand" href="{{ url('/') }}">{{ env('APP_NAME','TMPShareX') }}</a>
                ...
                <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                    <div class="ms-md-auto">
                        <div class="navbar-nav">
                            <a class="nav-link text-white" href="{{ route('home') }}"><span
                                    class="bi bi-house me-1"></span>Home</a>
                            <a class="nav-link text-white" href="{{ route('home.fileslist') }}"><span
                                    class="bi bi-folder me-1"></span>Files List</a>
                            <div class="nav-link text-white" role="button" data-bs-toggle="modal"
                                data-bs-target=".search-modal-front">
                                <span class="bi bi-search me-1"></span>Search
                            </div>
                            <a class="nav-link text-white" href="{{ route('upload.upload_page') }}"><span
                                    class="bi bi-upload me-1"></span>Upload</a>
                        </div>
                    </div>
                    <div class="navbar-nav">
                        @auth
                        <div class="nav-item dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end text-small">
                                ...
                                @if (Auth::user()->role_access == 'administrator')
                                <hr class="dropdown-divider">
                                <a class="dropdown-item" href="{{ route('admin.dashboard') }}"><span
                                        class="bi bi-speedometer2 me-1"></span>Admin
                                    Panel</a>
                                @endif
                                <hr class="dropdown-divider">
...
<div class="modal fade search-modal-front" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body text-center">
                <form action="{{ route('home.searchfiles') }}" method="GET">
                    <div class="input-group">
                        <input type="text" class="form-control" id="search" name="search" placeholder="Search Here">
                        <button type="submit" class="btn btn-primary"><span
                                class="bi bi-search me-1"></span>Search</button>
                    </div>
                </form>
            </div>
        </div>
